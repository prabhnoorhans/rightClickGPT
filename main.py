...
def post_audio():
    au_fl=open('/a/nApi/tmp.webm','wb')
    au_fl.write(request.files['audio'].read())
    au_fl.close()
    subprocess.run("ffmpeg -y -i /a/nApi/tmp.webm -ar 16000 /a/nApi/tmp.wav", shell=True, capture_output=True)
    result=subprocess.run("/a/main -m models/ggml-base.en.bin -f /a/nApi/tmp.wav", shell=True, capture_output=True)
    w_txt=result.stdout.decode().split(']')[1].strip()
    resp=jsonify({'text':w_txt})

    md_ctx = request.form.get("string")
    md_query = w_txt
